...
export class LlamaService {
  ...
  async initialize(config: ModelConfig): Promise<void> {
    ...
    try {
      console.log(
        "Initializing LLM with config:",
        config.modelPath,
        "n_ctx=",
        config.contextSize,
      );

      // KV cache compression (cache_type_k + cache_type_v) reduces memory for large context
      const cacheTypeK = config.cacheTypeK ?? "q4_0";
      const cacheTypeV = config.cacheTypeV ?? "q4_0";
      ...
      console.error("Failed to initialize LLM:", message, error);
  ...
  async completion(
    ...
    try {
      ...
      const prompt = ChatTemplate.applyTemplate(messages, { tools });
      ...
      generateChat: async (messages: any[], options?: any) => {
        ...
        const result = await this.completion(
          formattedMessages,
          {
            ...
            repeatPenalty: options?.repeatPenalty ?? 1.2,
            systemPrompt: "",
            contextSize: 4096,
            nGpuLayers: 0,
